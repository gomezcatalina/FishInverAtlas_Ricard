			...
			switch(i, 
				...
				3 == { 	# temperature distribution of catch 
					if(file.exists(paths[i])){
						...
						# graphics parameters
						#par(mfrow=c(1,1), omi=c(0,0,0,0), mai=c(0.35, 0.5+pos.ylabel[1], 0.1, 0.1), ps=8, cex=1, xpd=TRUE, lheight=0.5, xaxs="r", yaxs="r")
						par(mfrow=c(1,1), omi=c(0,0,0,0), mai=c(0.5, 0.75+pos.ylabel[1], 0.1, 0.1), ps=11, cex=1, xpd=TRUE, lheight=0.5, xaxs="r", yaxs="r")
				...
				9 == { 	# fish condition
					if(file.exists(paths[i])){
						...
						# graphics parameters
						#par(mfrow=c(1,1), omi=c(0,0,0,0), mai=c(0.35, 0.5+pos.ylabel[1], 0.1, 0.1), ps=12, cex=1, xpd=TRUE, lheight=0.5, xaxs="r", yaxs="r")
						par(mfrow=c(1,1), omi=c(0,0,0,0), mai=c(0.75, 0.8+pos.ylabel[1], 0.1, 0.1), ps=10, cex=1, xpd=TRUE, lheight=0.5, xaxs="r", yaxs="r")
