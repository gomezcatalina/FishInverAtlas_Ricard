...
#species.in <- read.csv(file.path(main.fp, "species-list-for-report.csv"), stringsAsFactors=FALSE, encoding="UTF-8", quote = '"')
main.path <- here::here()
...
## start by making the table that contains the taxonomic classification, which will establish the order in which the different species will appear in the Atlas
...
##
taxo.final <- taxo.df.out[,c("phylum","class","order","family","scientificname","comm.english","comm.fr","species.code","aphia.id","aphia.url")]
